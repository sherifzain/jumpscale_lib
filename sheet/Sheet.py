...
class Row(o.code.classGetBase()):
    ...
    def interpolate(self,start=None,stop=None,variation=0,min=None,max=None):
        ...
        try:
            interpolated=o.tools.numtools.interpolateList(tointerpolate,floatnr=self.nrfloat)
        except Exception,e:
    ...
    def randomVariation(self,variation,start=None,stop=None,min=None,max=None):
        ...
        for x in range(start,stop+1):
            self.cells[x]=self.cells[x]-variation/200+float(o.base.idgenerator.generateRandomInt(1,variation))/100
            if roundd:
    ...
    def _getVariationAbsoluteInt(self,val,variation):
        ...
        gd=o.base.idgenerator.generateRandomInt(changeMin,changeMax)
        return gd
    ...
    def _getVariationPositive(self,change,variation):
        ...
        gd=float(o.base.idgenerator.generateRandomInt(changeMin,changeMax)/100.0)
        return gd
    ...
    def _roundNrCumul(self,val,x,args):
        ...
        if nr+self._cumul>0.5 and nr+self._cumul<10:
            ...
            nr2=o.tools.numtools.roundDown(self._cumul*2,0)/2
            self._cumul+=nr-nr2
            ...
            nr2=o.tools.numtools.roundDown(nr,0)
            self._cumul+=nr-nr2
            return nr2
        else:
    ...
    def text2row(self,data,standstill=0,defval=None,startval=None,round=False,interpolate=None):
        ...
        def custom2rowvalues(datas):
                ...
                if not(str(data).find(",")==-1 and str(data).find(":")==-1):
                    ...
                    for item in splitted:
                        ...
                        if start==None and interp:
                            start=(int(o.tools.numtools.roundDown((float(pos)/12))))*12
                        try:
                            value=o.tools.numtools.text2val(value)
                        except Exception,e:
                            print "error: %s " %e
            ...
            if maxpos==None:
                ...
                stop=int(o.tools.numtools.roundUp((float(maxpos)/12)))
            if start==None:
                start=0
        ...
        if not o.basetype.list.check(data):
            if data==None:
                self.cells[0]=float(startval)
...
class Sheet(o.code.classGetBase()):
    ...
    def _dict2obj(self,dict):
        ...
        for key in dict["rows"].keys():
            item=dict["rows"][key]
            row=o.code.dict2object(Row(),item)
            ...
            for key in range(len(result)):
                if headerDone==False:
                    if period=="Y":
                        ...
                        year=o.tools.numtools.roundDown(float(key)/4)
                        Q="Q%s" % int(o.tools.numtools.roundDown(float(key-1-year*4))+2)
                        year="Y%s" % int(year+1)
                        hid="%s %s" % (year,Q)
                    header.append(hid)
    ...
    def getSheetAggregated(self,period="Y"):
        ...
        lenx=len(headers)-1
        sheet2=o.tools.sheet.new(self.name,self.nrcols,headers)
        sheet2.description=self.description
        sheet2.remarks=self.remarks
        sheet2.period=period
        for row in rows:
            roworg=roworg=self.getRow(row[0])
            rownew=sheet2.addRow(roworg.name, roworg.ttype, roworg.aggregateAction, roworg.description\
                          , roworg.groupname, roworg.groupdescr, lenx, \
    ...
    def getRow(self,rowName):
        if not self.rows.has_key(rowName):
    ...
    def applyFunction(self,rowNames,method,rowNameDest="",params={}):
        ...
        if not self.rows.has_key(rowNameDest):
    ...
    def addRows(self,rows2create,aggregation):
        ...
        for group in rows2create.keys():
            ...
            for rowname in rownames:
                if aggregation.has_key(rowname):
    ...
    def sumRows(self,rownames,newRow):
        ...
        """
        if o.basetype.string.check(newRow):
