...
class USB:
    ...
    def _prereq(self):
        if not o.system.fs.exists(self._USB_DEVICE_PATH):
               o.system.fs.createDir(self._USB_DEVICE_PATH)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if filename:
            if filename.find("/") != -1:
                filename = o.system.fs.getBaseName(filename)
            ...
            if not o.system.fs.exists(devicename):
    ...
    def remove(self):
       try:
           o.system.fs.removeFile(self._filename)
       except Exception as ex:
...
class USBExtension:
    ...
    def _parseDF(self, usb):
        ...
        try:
            ...
            for partition in partitions:
                pmountpoint = o.system.fs.joinPaths(usb.mountpoint, "%s%s" % (o.system.fs.getBaseName(usb.devicename), partition['number']))
                command          = 'df -m {0}'.format(pmountpoint)
                exitcode, output = o.system.process.execute(command)
                dictOfSpace = {}
                if output:
    ...
    def getFreeSpace(self, usb):
        ...
        if spaceInfo:
            ...
            for partition in spaceInfo:
                partitionName = o.system.fs.getBaseName(partition.get('device',""))
    ...
    def format(self, usb, partitionnr=1, fstype = 'ntfs', label = None, compression = False):
        ...
        device  = o.system.fs.getBaseName(usb.devicename)

        if not o.cmdtools.disktools.parted.getPartitions(device):
        ...
        if not o.system.fs.exists(device):
        ...
        command = 'mkfs -t {0} {1} {2} {3}'.format(fstype,
                                                   ...
                                                   device)
        try:
            exCode, out = o.system.process.execute(command = command)
    ...
    def partition(self, usb, size=None, fsType = 'NTFS'):
        ...
        try:
            if self._verifyDevice(usb):
                device = o.system.fs.getBaseName(usb.devicename)
                partitions = o.cmdtools.disktools.parted.getPartitions(device)
                size = int(size) * 1024 if size else None
 
                freeSpace  = o.cmdtools.disktools.parted.getFreeSpace(device)
                ...
                exCode, output = o.system.process.execute(command)
    ...
    def getFilesystem(self, usb):
        ...
        if not partitions:
            return fsInfo
        for partition in partitions:
            partitionName = "%s%s" % (o.system.fs.getBaseName(usb.devicename), partition['number'])
    ...
    def list(self):
        ...
        usbs  = []
        try:
            o.system.fswalker.walk(self._USB_DEVICE_PATH.format(''),dirlister,paths,recursive=False)
        except Exception as ex:
    ...
    def mount(self, usb):
        ...
        if not self.isMounted(usb):
            try:
                if not o.system.fs.exists(self._MOUNTPOINT.format("")):
                    o.system.fs.createDir(self._MOUNTPOINT.format(""))
                ...
                for partition in partitions:
                    ...
                    pmountpoint = o.system.fs.joinPaths(mountpoint, "%s%s"%(o.system.fs.getBaseName(usb.devicename), partition['number']))
                    ...
                    if not o.system.fs.exists(device):
                        ...
                        return False
                    if not pmountpoint in o.system.fs.listDirsInDir(self._MOUNTPOINT.format('') ):
                        o.system.fs.createDir(pmountpoint)
                    if fsType:
                        o.cmdtools.disktools.mount(device, pmountpoint, fsType)
                    else:
                        o.cmdtools.disktools.mount(device, pmountpoint)
    ...
    def umount(self, usb):
        ...
        for partition in partitions:
            pmountpoint = o.system.fs.joinPaths(mountpoint, "%s%s" % (o.system.fs.getBaseName(usb.devicename), partition['number']))
            if not o.system.fs.isMount(pmountpoint):
                continue
            try:
                o.cmdtools.disktools.umount(pmountpoint)
            except Exception as ex:
                self._LOGGER.log("Unmount of device {0}, partition {1} failed with exception {2}. Trying a lazy and forced unmount".format(usb.label,partition['number'], ex))
                try:
                    o.cmdtools.disktools.umount(pmountpoint,'-l -f')
                except Exception as ex2:
                    self._LOGGER.log("Unmount of device {0} partition {1} failed with exception {2} while trying a lazy forced unmount".format(usb.label,partition['number'], ex2))
                    return False
            try:
                o.system.fs.removeDir(pmountpoint)
            except Exception as ex:
    ...
    def isMounted(self, usb):
        ...
        for partition in partitions:
            pmountpoint = o.system.fs.joinPaths(mountpoint, "%s%s" % (o.system.fs.getBaseName(usb.devicename), partition['number']))
            if o.system.fs.isMount(pmountpoint):
                return True
        return False
    ...
    def getPartitionInfo(self, usb):
        ...
        @rtype                : list
        """
        return o.cmdtools.partitioninfo.infoParted(usb.devicename[5:])
    ...
    def _parseSerial(self, usb):
        ...
        exitCode, serial = o.system.process.execute(command              = command,
    ...
    def _verifyDevice(self, usb, strict = False):
        ...
        if serial != usb.serial:
            ...
            if strict:
                return false
        return o.system.fs.exists(usb.devicename)
