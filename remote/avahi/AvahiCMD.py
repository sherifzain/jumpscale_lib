...
class AvahiCMD():
    ...
    def getServices(self):
        ...
        result,output=o.system.process.execute(cmd,False,False)
        if result>0:
        ...
        items=o.codetools.regex.extractBlocks(output,["^= .*"])
        ...
        for item in items:
            ...
            s.hostname=o.codetools.regex.getINIAlikeVariableFromText(" *hostname *",item).replace("[","").replace("]","").strip()
            s.address=o.codetools.regex.getINIAlikeVariableFromText(" *address *",item).replace("[","").replace("]","").strip()
            s.port=o.codetools.regex.getINIAlikeVariableFromText(" *port *",item).replace("[","").replace("]","").strip()
            s.txt=o.codetools.regex.getINIAlikeVariableFromText(" *txt *",item).replace("[","").replace("]","").strip()
    ...
    def resolveAddress(self, ipAddress):
        ...
        """
        #do some validation
        if not o.system.net.validateIpAddress(ipAddress):
            raise ValueError('Invalid Ip Address')
        cmd = 'avahi-resolve-address %s'
        exitCode, output = o.system.process.execute(cmd%ipAddress, dieOnNonZeroExitCode = False, outputToStdout = False)
