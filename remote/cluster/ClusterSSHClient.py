...
class ClusterSSHClient():
    ...
    def connect(self,force=False,testonly=False):
        ...
        if self.isconnected==True and force==False:
            return True
        if not o.system.net.pingMachine(self.node.ipaddr,5):
        ...
        o.logger.log( "connect %s" % self.node.ipaddr)
        state="start"
        passwdworking = None
        if self.passwd<>"":
        ...
        for passwd in passwords2try:
            if state=="start" or state=="autherror":
                try:
                    ...
                    #o.logger.log("clustersshclient: try to login with passwd %s on %s" %(passwd,self.node.ipaddr),2)
                    self._sshclient=o.remote.system.connect(self.node.ipaddr,"root",passwd,timeout=10)
                    state="done"
                    o.logger.log("cluster node %s connected over ssh" % self.node.ipaddr,5)
                    passwdworking=passwd
                    
                except o.remote.system.exceptions.RemoteSystemAuthenticationError:#:AuthenticationException,e:
                    state="autherror"
                    o.logger.log("Could not sshlogin into %s with login %s, passwd %s" % (self.node.ipaddr,"root",passwd))
                #except Exception, e:
    ...
    def getSFtpConnection(self):
        o.logger.log("Open SFTP connection to %s" % (self.node.ipaddr))
