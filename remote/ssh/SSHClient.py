...
class SSHClient:
    ...
    def executewait(self, command,dieOnError=True):
        ...
        o.logger.log("Execute ssh command %s on %s" % (command,self.host))
    ...
    def getSFtpConnection(self):
        o.logger.log("Open SFTP connection to %s" % (self.host))
    ...
    def _removeRedundantFiles(self,path):
        ...
        files.extend(o.system.fs.listFilesInDir(path,True,filter="*.pyo")) #@todo remove other files  (id:6)
        for item in files:
